...
int main()
{
    ...
    userInterfaceSkill EradicatorStrike = userInterfaceSkill(900,410,950,460,MAGENTA,ERADICATE,1,frame,2);
    ...
    while (true)
    {
        ...
        if (WM_LBUTTONDOWN && !Paralysis.isInEffect() && EradicatorStrike.available() && gamer.getMana()>=EradicatorStrike.getManaCost() && gamer.getAuto())
        {
        ...
        if (kbhit() && gamer.getHealth()>0)
        {
            ...
            if (c==59)
            {
                ...
                outtextxy(500,0,"GUIDE");
                outtextxy(100,600,"PRESS ENTER TO EXIT");
                getch();
            }
        }
    }
